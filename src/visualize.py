...
def plot_temporal_conditioning(model, test_loader, T_audio, max_rate, device, out_dir,
    ...
    for row in range(n_show):
        for col, t_val in enumerate(t_values):
            ...
            ax = axes[row, col]
            ax.plot(x0[row].cpu(), color="C0", alpha=0.5, linewidth=0.9, label="clean")
            ax.plot(noisy[0].cpu(), color="C1", alpha=0.55, linewidth=0.8, label="noisy")
            ax.plot(den[0].cpu(), color="C2", alpha=0.9, linewidth=1.0, label="denoised")
